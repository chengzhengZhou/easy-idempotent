...
public class MemoryIdempotentRecordRepoImpl implements IdempotentRecordRepo {
    /** 日志记录器 */
    private final Logger logger = LoggerFactory.getLogger(MemoryIdempotentRecordRepoImpl.class);

    Map<String, IdempotentRecord> cache = new ConcurrentHashMap<>();

    @Override
    public boolean add(String key, String value, int expireTime, TimeUnit timeUnit) {
        logger.debug("memory add {}", key);
